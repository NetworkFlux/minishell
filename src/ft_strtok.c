...
/*   Updated: 2022/02/24 17:22:38 by fcaquard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_strtok(s_cmd_t *s_cmd, size_t start, int is_command)
{
	while (start < ft_strlen(s_cmd->s_cmd))
	{
		...
		{
			if (!parse_block(s_cmd, &start, &is_command))
				return (0);
		}
		// else if (is_delimiter(s_cmd->s_cmd[start]))
		// {
		// 	if (!parse_delimiter(s_cmd, &start, &is_command))
		// 		return (0);
		// }
		else
		{
			if (!parse_param(s_cmd, &start, &is_command))
				return (0);
		}
...
int	parse_alt(f_cmd_t *f_cmd)
{
	...
	while (i < f_cmd->nb_scmd && f_cmd->s_cmd[i])
	{
		...
		// if (!count_input(current, 0) || current->ntokens == 0 || current->ncmd == 0)
